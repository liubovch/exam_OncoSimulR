...
In this model from @Tomlinson1997, we consider 3 strategies, each of them corresponding to a different mutant genotype:
 ...
 3. Cells do not produce any growth factor but benefit from the factor produced by cells of type A.  
...
We start from a WT population of 5000 cells with a mutation rate of 1e-5 to see faster the dynamics of the tumor cells evolution.
...
There is strong selection for autocrine growth factor production under this model as stated in @Tomlinson1997, as long as the benefit outweighs the cost (c > 0). For example, in a situation in which the benefit of the paracrine factor is bigger than the benefit of the autocrine one, strategy b will also win since its fitness has the contribution of both of them.
...
# a <- 1,b <- 3, c <- 2 (from previous example)

plot(osi, show = "genotypes", type = "line")


# Model just with B and C interactions
...
In this model , taken also from @Tomlinson1997, there are 2 type of mutants:
...
Results from the simulations do not quite match the main theoretical insight extracted from @Tomlinson1997. For them, as long as the benefit j of angiogenesis is greater than the cost i of producing angiogenic factors then both types of strategies will be present in a tumour in a polymorphic equilibrium. In our case, running 100 simulations,  we have reached such equilibrium 30% of the times, while strategy 1 wins 26%.


```{r barplot, eval=TRUE, echo=FALSE, fig.cap="Winner strategies 100 simulations"}
knitr::include_graphics("angiogenesis_1.png")
```


In the case in which the cost is bigger than the benefit (i.e. i = 2,  j = 1), the result is the advantage of strategy 2 as stated by @Tomlinson1997, although there is an important fraction of the simulations in which mutants do nott get to outgrow WT cells:


```{r barplot, eval=TRUE, echo=FALSE, fig.cap="Winner strategies 100 simulations"}
knitr::include_graphics("angiogenesis_2.png")
```
...
According to @Tomlinson1997, the outcome of this model depends a lot on the initial population of the mutants. In our case we can just specify a number of initial WT individuals with no mutations present. It will be interesting to be able to compare results starting from different distributions of population.



#### Stochastic Tunnelling  {#tunnelling}


Here is a small example inspired by @Komarova2003 in which inactivation of a tumor supressor gene in modelled. We have three types of cells:

1. **WT** have 2 intact copies of the gene.
2. **A** have 1 intact copy of the gene.
3. **AB** have both copies of gene inactivated.

The mutation-selection network is as follows: $$WT \rightarrow A \rightarrow AB$$

The probability that a cell of type **WT** reproduces is proportional to its frequency ($f$) and the "reproductive rate" ($r$) (or, more precisely, to its relative probability 
to be chosen for reproduction):
$$ \frac{r_{WT}f_{WT}}{r_{WT}f_{WT} + r_Af_A + r_{AB}f_{AB}} $$
...
## Here we also specify explicitely "B" cells with their fitness equal to 0
r <- data.frame(Genotype = c("WT", "A", "B", "A, B"),
...
In a *genuine two-step process*, starting from the all **WT** state, the system gets to the state where the entire population consists of cells of type **A** and finally reaches fixation in the all **AB** state:

```{r}
## Genuine two-step process
set.seed(5)  ## for reproducibility
osi <- oncoSimulIndiv(afe,
                      model = "McFL",
                      onlyCancer = FALSE,
                      finalTime = 100000,
                      mu = c("A" = 1e-4, "B" = 1e-3),  ## "A" = 2 * 0.5e-4 (two alleles)
                      initSize = 100,
                      seed = NULL,
                      detectionProb = NA,
                      detectionSize = NA,
                      errorHitMaxTries = FALSE,
                      errorHitWallTime = FALSE,
                      sampleEvery=1)  ## to speed-up the simulation 
plot(osi, show = "genotypes", type = "line")

```


But for some parameters range (for example, when $\mu_{AB} \gg 1/N$, where $N$ is a population size), the *tunnelling* can prevail. This means that from the all **WT** state the system gets absorbed into the all **AB** state, skipping the intermediate fixation of type **A**.
...
### Cytotoxic interactions between tumor cells {#Cytotoxicity}
...
This model has been taken from @Sartakhti2016, there are 3 strategies(or genotypes):
...
plot(osi, show = "genotypes", type = "line", xlim= c(12, 800))
...
afe <- fast_allFitnessEffects(r)
osi <- fast_oncoSimulIndiv(afe)
plot(osi, show = "genotypes", type = "line", xlim= c(12, 1000))
...
For this model that has been obtained from @Tomlinson1997, since all the celltypes represent different phenotypes, the fitness functions have been set as follows:
FA = 1 + bz*f_C + ay*f_B            
FB = 1 + ax*f_A - dz*f_C
FC = 1 + bx*f_A
...
The expected result is that the final frequency of OC will be greater than the final frequency of OB and which will also be greater than the final frequency of MM.
...
afe <- fast_allFitnessEffects(r)
osi <- fast_oncoSimulIndiv(afe)
plot(osi, show = "genotypes", type = "line", xlim= c(12, 20))
...
afe <- fast_allFitnessEffects(r)
osi <- fast_oncoSimulIndiv(afe)
plot(osi, show = "genotypes", type = "line", xlim= c(12, 40))
```
...
afe <- fast_allFitnessEffects(r)
osi <- fast_oncoSimulIndiv(afe)
plot(osi, show = "genotypes", type = "line", xlim= c(12, 23))
```
